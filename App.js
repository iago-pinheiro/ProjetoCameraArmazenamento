...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Solicita permissão de acesso para leitura e escrita na galeria de fotos (expo-media-library)
  ...
  const takePhoto = async () => {
    // Verifica se as permissões foram concedidas antes de abrir a câmera
    if (!hasCameraPermission) {
      Alert.alert("Erro", "Permissão de câmera não concedida.");
      return;
    }
    // Define o modo da câmera como foto e exibe o preview em tela
    setCameraMode("photo");
    setShowCamera(true);
  };

  // Função auxiliar chamada ao pressionar o botão de captura no preview da câmera (modo foto)
  const handleTakePhoto = async () => {
    // Verifica se a referência à câmera está disponível
    if (!cameraRef.current) return;

    try {
      // Captura a foto usando a vision-camera, passando o modo de flash selecionado
      const photo = await cameraRef.current.takePhoto({
        flash: flash, // Aplica o modo de flash configurado pelo usuário
      });

      // Monta a URI completa do arquivo de foto capturado
      const uri = "file://" + photo.path;
      // Salva a URI da foto no estado principal do app
      setImage(uri);
      // Salva o tipo da mídia como imagem
      setMediaType("image");
      // Fecha o preview da câmera após a captura
      setShowCamera(false);

      try {
        // Tenta criar o asset de mídia na galeria física e pública do celular
        await MediaLibrary.createAssetAsync(uri);
        // Exibe o alerta de sucesso após o salvamento
        Alert.alert("Sucesso", "Imagem salva na galeria!");
      } catch (error) {
        // Exibe o alerta de erro em caso de falha no salvamento
        Alert.alert("Erro", "Erro ao salvar a imagem.");
      }
    } catch (error) {
      // Captura erros inesperados durante a captura da foto
  ...
  const recordVideo = () => {
    ...
    // Define o modo da câmera como vídeo e exibe o preview em tela
    setCameraMode("video");
    setShowCamera(true);
  };

  // Função auxiliar chamada ao pressionar o botão de iniciar gravação no preview da câmera (modo vídeo)
  const handleStartRecording = async () => {
    // Verifica se a referência à câmera está disponível
    if (!cameraRef.current) return;

    // Ativa o indicador de gravação em andamento (feedback visual vermelho)
    setIsRecording(true);

    // Inicia a gravação de vídeo usando a vision-camera com callback de conclusão
    cameraRef.current.startRecording({
      // Callback chamado automaticamente quando a gravação for parada ou falhar
      onRecordingFinished: async (video) => {
        ...
        setMediaType("video");
        // Desativa o indicador de gravação
        setIsRecording(false);
        // Fecha o preview da câmera após a gravação
        setShowCamera(false);
        ...
        try {
          ...
          // Alerta o usuário caso ocorra uma falha ao salvar na galeria
  ...
  const handleStopRecording = async () => {
    // Verifica se a referência à câmera está disponível
    if (!cameraRef.current) return;
    // Para a gravação de vídeo em andamento (o callback onRecordingFinished será chamado automaticamente)
  ...
  const pickImage = async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      ...
      quality: 1,                                       // Mantém a qualidade máxima da imagem
    });

    // Se o usuário selecionou uma imagem com sucesso e não cancelou
    if (!result.canceled) {
      // Obtém o endereço local da imagem selecionada
      const uri = result.assets[0].uri;
      // Salva a URI da imagem no estado principal
      setImage(uri);
  ...
  const pickVideo = async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      ...
      quality: 1,                                       // Mantém a qualidade máxima do vídeo
    });

    // Se o usuário selecionou um vídeo com sucesso e não cancelou
    if (!result.canceled) {
      // Obtém o endereço local do vídeo selecionado
      const uri = result.assets[0].uri;
      // Salva a URI do vídeo no estado principal
      setImage(uri);
  ...
  return (
    // View principal que centraliza o conteúdo de acordo com o stylesheet
    <View style={styles.container}>

      {/* Condição: Se o preview da câmera deve ser exibido (ao tirar foto ou gravar vídeo) */}
      {showCamera && device ? (
        // View que ocupa toda a tela para exibir o preview da câmera em tempo real
        <View style={styles.cameraContainer}>
          ...
          <Camera
            ...
            audio={cameraMode === "video"} // Habilita o áudio somente no modo de vídeo
          ...
          <View style={styles.cameraControls}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                // Se estiver gravando, para a gravação antes de fechar
                if (isRecording) {
          ...
          <View style={styles.captureContainer}>
            ...
            {/* Condição: Se o modo for foto, exibe botão de tirar foto */}
            {cameraMode === "photo" && (
            ...
            {/* Condição: Se o modo for vídeo e não estiver gravando, exibe botão de iniciar */}
            ...
            {/* Condição: Se estiver gravando, exibe botão de parar com feedback visual vermelho */}
            {cameraMode === "video" && isRecording && (
              <TouchableOpacity
          ...
          {/* Feedback visual: texto de aviso exibido durante a gravação de vídeo */}
          {isRecording && (
        ...
        <View style={styles.mainContent}>
          ...
          {/* Condição: Se houver mídia carregada e ela for uma foto, exibe-a com a tag Image */}
          {image && mediaType === "image" && (
            <Image source={{ uri: image }} style={styles.image} />
          )}

          {/* Condição: Se for vídeo (gravado ou selecionado da galeria), exibe mensagem informativa na tela */}
...
const styles = StyleSheet.create({
  ...
  mainContent: {
    ...
    padding: 16,              // Adiciona um espaçamento interno de 16 pixels nas bordas
  },

  // Estilo do título original
  title: {
    fontSize: 22,             // Define tamanho da fonte do título como 22 pontos
    fontWeight: "bold",       // Deixa a fonte em negrito
    marginBottom: 20,         // Margem inferior de 20 pixels para separar do botão
  },

  // Estilo original da imagem capturada
  image: {
  ...
  cameraControls: {
    position: "absolute",     // Posiciona os botões sobre o preview
  ...
  // Estilo individual dos botões de controle sobrepostos
  controlButton: {
  ...
  // Estilo adicional para o botão de cancelar (cor diferenciada)
  cancelButton: {
    backgroundColor: "rgba(200,0,0,0.7)", // Fundo vermelho semitransparente para indicar cancelamento
  },

  // Estilo do texto dos botões de controle sobrepostos
  controlButtonText: {
  ...
  captureContainer: {
    position: "absolute",     // Posiciona o botão sobre o preview
  ...
  captureButtonText: {
    fontSize: 16,             // Tamanho da fonte adequado
    fontWeight: "bold",       // Texto em negrito
    color: "black",           // Texto preto para contraste
  },

  // Estilo do indicador de gravação em andamento (feedback visual)
  recordingIndicator: {
  ...
  // Estilo do texto do indicador de gravação
  recordingText: {
    color: "red",             // Texto vermelho para indicar gravação em andamento
